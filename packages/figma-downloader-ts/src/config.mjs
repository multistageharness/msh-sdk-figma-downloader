...
export function loadConfigFile(args = {}, { cwd = process.cwd() } = {}) {
  ...
  const file = explicit ? path.resolve(cwd, explicit) : path.resolve(cwd, DEFAULT_CONFIG_FILENAME);
  try {
...
export function resolveNetworkConfig(args = {}, env = process.env, opts = {}) {
  ...
  if (fileCfg.proxy !== undefined) {
    proxy = fileCfg.proxy === null || fileCfg.proxy === '' ? null : String(fileCfg.proxy);
