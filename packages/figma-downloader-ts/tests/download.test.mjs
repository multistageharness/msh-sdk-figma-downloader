...
test("util: batch / pool / countNodes / sha256OfBatch", async () => {
  ...
  // content hash is order-independent and stable
  assert.equal(sha256OfBatch(["b", "a"]), sha256OfBatch(["a", "b"]));
  assert.equal(sha256OfBatch(["a", "b"]).length, 12);
});

test("tokenBucket: gates acquisitions to the configured rate", async () => {
  let clock = 0;
  const tb = tokenBucket({ rate: 1000, burst: 2, now: () => clock });
  await tb.acquire();
  await tb.acquire();
  // two tokens consumed; capacity adjusts
  tb.setCapacity(1);
  assert.equal(tb.capacity, 1);
});
...
test("client: fetchSkeleton truncates, fetchNodes returns subtrees, fetchWholeFile is whole", async () => {
  ...
  try {
    ...
    const whole = await fetchWholeFile("KEY", "tok", {});
    assert.deepEqual(whole.document, MOCK_FILE.document);
  } finally {
...
test("reconstruct: grafted file matches the original full tree", async () => {
  ...
  try {
    ...
    const { file, stats } = reconstruct(skel, indexParts(parts), {
      frontierIds: ids,
    });
...
test("reconstruct: records missing frontier nodes without dropping them", () => {
  ...
  const { file, stats } = reconstruct(skel, indexParts([]), {
    frontierIds: ["1:1"],
  });
...
test("cli end-to-end: --mock writes a full.json equal to the source", async () => {
  ...
  assert.equal(manifest.size, "md");
  assert.ok(
    manifest.parts.every((p) => /^part-[0-9a-f]{12}\.json$/.test(p.file)),
    "content-hashed part names",
  );

  await fs.rm(dir, { recursive: true, force: true });
});

test("cli: --size sm takes the whole-file fast path (no parts)", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "figdl-"));
  const code = await run([
    "--mock",
    "--size",
  ...
  assert.equal(manifest.size, "sm");
  assert.equal(manifest.mode, "whole");
  await assert.rejects(
    fs.access(path.join(dir, "parts")),
    "no parts dir on the whole-file path",
  );
  await fs.rm(dir, { recursive: true, force: true });
});
...
test("cli: --resume skips already-present groups", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "figdl-"));
  await run(["--mock", "--work-dir", dir, "--keep-parts", "--quiet"]);
  const code = await run([
    "--mock",
  ...
  assert.ok(
    ...
    "resume should skip at least one group",
  );
  const full = JSON.parse(
    await fs.readFile(path.join(dir, "full.json"), "utf8"),
  );
...
// --- Proxy / TLS transport (makeNodeFetch). ---
