...
export async function figmaGet(pathAndQuery, token, opts = {}) {
  if (!token)
    throw new FigmaError("FIGMA_TOKEN is required (or use --mock).", {
      exitCode: 3,
    });
  ...
  while (true) {
    ...
    try {
      ...
      const transient =
        aborted || isNetwork || (err instanceof FigmaError && err.retryable);
      if (!transient || attempt >= retries) {
        if (aborted)
          throw new FigmaTimeoutError(
            `Figma API timed out after ${timeoutMs / 1000}s for ${pathAndQuery}`,
          );
        if (err instanceof FigmaError) throw err;
        throw new FigmaNetworkError(
          `Figma API network error for ${pathAndQuery}: ${String(err).slice(0, 120)}`,
        );
      }
...
export async function fetchSkeleton(fileKey, token, opts = {}) {
  ...
  const version = opts.version
    ...
    : "";
  return figmaGet(
    `/files/${encodeURIComponent(fileKey)}?depth=${depth}${geometry}${version}`,
    token,
    opts,
  );
}
...
export async function fetchNodes(fileKey, ids, token, opts = {}) {
  ...
  const version = opts.version
    ...
    : "";
  return figmaGet(
    `/files/${encodeURIComponent(fileKey)}/nodes?ids=${idsParam}${depthQ}${geometry}${version}`,
    token,
    opts,
  );
}
