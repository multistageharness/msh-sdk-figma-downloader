...
import { exitCodeFor, FigmaError } from "./errors.mjs";
import {
  _http,
  FIGMA_API_BASE,
  fetchSkeleton,
  fetchWholeFile,
  parseFileKey,
} from "./figmaClient.mjs";
import { makeNodeFetch, redactProxy } from "./httpFetch.mjs";
import { makeMockFetch } from "./mock.mjs";
import { indexParts, loadParts, reconstruct } from "./reconstruct.mjs";
import { classify, resolveStrategy, TIERS } from "./sizing.mjs";
import { byteLength, countNodes, stringify, tokenBucket } from "./util.mjs";
...
async function runInner(argv, { env }) {
  ...
  if (sizeArg !== "auto" && !TIERS.includes(sizeArg)) {
    process.stderr.write(
      `error: --size must be one of auto|${TIERS.join("|")}\n`,
    );
    return 2;
  }
  const explicit = {
  ...
  const writeFull = async (file, stats, sizeLabel) => {
    ...
    if (args.stdout) {
      ...
      log(
        `wrote ${outPath} — ${countNodes(file.document)} node(s)${stats ? `, ${stats.grafted} subtree(s) grafted` : ""} [${sizeLabel}]`,
      );
    }
  ...
  if (args["reconstruct-only"]) {
    ...
    try {
      const m = JSON.parse(
        await fs.readFile(path.join(workDir, "manifest.json"), "utf8"),
      );
    ...
    const { file, stats } = reconstruct(skeleton, indexParts(parts), {
      frontierIds,
    });
  ...
  if (args.mock) {
    ...
    _http.fetch = makeNodeFetch({
      proxy: net.proxy || null,
  ...
  if (
    env.FIGMA_HTTP_TIMEOUT_MS !== undefined &&
    String(env.FIGMA_HTTP_TIMEOUT_MS).trim() !== ""
  ) {
    const n = Number(env.FIGMA_HTTP_TIMEOUT_MS);
    if (!Number.isFinite(n))
      throw new Error(
        `FIGMA_HTTP_TIMEOUT_MS must be a number (got ${env.FIGMA_HTTP_TIMEOUT_MS})`,
      );
    timeoutMs = n;
    timeoutSource = "env";
  }
  ...
  if (!args.mock && !net.sslVerify) {
    process.stderr.write(
      `[figma-download] WARNING: TLS certificate verification is DISABLED (from ${net.sources.sslVerify}).\n`,
    );
  }
  if (!args.mock && !token) {
    process.stderr.write(
      `[figma-download] WARNING: no Figma token found (FIGMA_TOKEN / FIGMA_API_TOKEN / FIGMA_ACCESS_TOKEN); live requests will fail with 403.\n`,
    );
  }
  ...
  try {
    const reqOpts = {
      ...
      onRetry: ({ status, attempt, waitMs, error }) =>
        log(
          `  retry #${attempt} (${status === "network" ? `connection issue: ${error}` : `status ${status}`}) in ${waitMs}ms`,
        ),
    };

    // --- Classify size: probe a skeleton, then resolve the strategy. ---
    const probeDepth = explicit.depth
      ? intOpt(args, "depth", 2)
      : sizeArg !== "auto"
        ? undefined
        : 2;
    ...
    if (sizeArg === "auto") {
      ...
      skeleton = await fetchSkeleton(fileKey, token, {
        ...reqOpts,
        depth: probeDepth ?? 2,
      });
      ...
      strategy = resolveStrategy({
        ...
        maxPartBytes:
          args["max-part-bytes"] !== undefined
            ? intOpt(args, "max-part-bytes", 0)
            : undefined,
        maxPartMs:
          args["max-part-ms"] !== undefined
            ? intOpt(args, "max-part-ms", 0)
            : undefined,
        explicit,
      });
      log(
        `classified ${tier} (nodes≈${nodeCount}, frontier=${frontier.length}, skeleton=${skeletonBytes}B)`,
      );
    ...
    log(
      `skeleton: "${skeleton.name}" v${skeleton.version} (size=${strategy.size}, depth=${depth})`,
    );
    ...
    const limiter =
      rate > 0
        ? tokenBucket({ rate, burst: Math.max(1, strategy.concurrency) })
        : null;

    log(
      `downloading ${Math.ceil(frontierIds.length / strategy.batchSize)} batch(es), ${strategy.batchSize} id(s) each, concurrency ${strategy.concurrency}${rate ? `, ${rate} req/s` : ""}`,
    );
