...
import { batch, byteLength, pool, sha256OfBatch, stringify } from "./util.mjs";
...
export async function downloadChunks({
  ...
  const splitNode = async (node, level, maps, groupFiles) => {
    ...
    for (const child of kids)
      await splitNode(child, level + 1, maps, groupFiles);
  };
  ...
  const tasks = batches.map((ids, bi) => async () => {
    ...
    if (resume && priorGroups.has(gsha)) {
      ...
      const present =
        parts.length &&
        (
          await Promise.all(parts.map((f) => exists(path.join(partsDir, f))))
        ).every(Boolean);
      if (present) {
        skipped += 1;
    ...
    const data = await fetchNodes(fileKey, ids, token, {
      ...
      onRetry: ({ status, attempt, waitMs }) =>
        log(
          `  [${bi + 1}/${batches.length}] retry #${attempt} (status ${status}) in ${waitMs}ms`,
        ),
    });
